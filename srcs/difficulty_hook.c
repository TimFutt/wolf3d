...
int		mouse_clic_difficulty(int b, int x, int y, t_mlx *e)
{
	...
	if (e->choice == 4)
	{
		...
		if ((b == 1) && x >= 424 && x <= 601 && y >= 290 && y <= 344)
			...
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/CLASSICCLIC.xpm", &i, &i);
		else if ((b == 1) && x >= 317 && x <= 695 && y >= 541 && y <= 595)
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/HARDCORECLIC.xpm", &i, &i);
		else if ((b == 1) && x >= 47 && x <= 218 && y >= 702 && y <= 735)
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/RETURNCLIC2DIFFICULTY.xpm", &i, &i);
		else
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/DIFF.xpm", &i, &i);
...
int		mouse_release_difficulty(int b, int x, int y, t_mlx *e)
{
	...
	if (e->choice == 4)
	{
		...
		{
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/CLASSICSURVOL.xpm", &i, &i);
			e->diff = 2;
		}
		else if ((b == 1) && x >= 317 && x <= 695 && y >= 541 && y <= 595)
		{
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/HARDCORESURVOL.xpm", &i, &i);
			e->diff = 3;
		}
		else if ((b == 1) && x >= 47 && x <= 218 && y >= 702 && y <= 735)
		{
			e->data = mlx_xpm_file_to_image(e->ptr,
				"img/xpm/RETURNSURVOL2DIFFUCULTY.xpm", &i, &i);
			e->choice = 3;
		}
		else
			e->data = mlx_xpm_file_to_image(e->ptr,
